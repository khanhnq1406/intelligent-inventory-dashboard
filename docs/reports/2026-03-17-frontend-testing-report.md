...
| 2026-03-18 | Fixed 8 E2E test failures: (1) `dashboard.spec.ts` — stats cards need 15s timeout for API data load; (2) `dashboard.spec.ts` — use `getByRole('heading')` to avoid strict mode violation when "Recent Actions" text also appears in empty table cell; (3) `aging.spec.ts` — removed ambiguous `/vehicles/i` regex matcher (matched 2 elements), kept only `"require attention"` check; (4) `inventory.spec.ts` + `vehicle-detail.spec.ts` — wait for `td.font-mono` VIN cells (real data rows, not loading skeletons) before clicking row, and extend navigation timeout to 10s | Minor | — |
